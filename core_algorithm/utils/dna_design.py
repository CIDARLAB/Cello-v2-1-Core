...
class DevChain:
    ...
    # excluding internal rules (already validated against these)
    after: list[str]
    # excluding internal rules (already validated against these)
    before: list[str]
...
class DNADesign:
    ...
    def __init__(self, structs, cassettes, sequences, device_rules, circuit_rules, fenceposts):
        ...
        # contains the input, output, components, color, dev_rules, and cir_rules (critical)
        self.cassettes = cassettes
        # contains the part type and actual dna sequence; also contains scars
        self.sequences = sequences
        # rules that govern the short sequences of parts that make up devices
        self.device_rules = device_rules
        # rules that govern the broader order of those devices
        self.circuit_rules = circuit_rules
        # contains the fenceposts/genetic locations that become the nonce_pads
    ...
    def get_part_orders(self):
        ...
        selected_device_orders = call_mini_eugene(
            self.circuit_rules)  # NOTE: miniEugene*
    ...
    # equivalent of 2.0 dpl_part_information.csv
    def write_dna_parts_info(self, filepath) -> None:
        """
        ...
        with open(dna_part_info_path, 'w', newline='') as dna_part_info:
            ...
            for seq in self.sequences.values():
                ...
                lab = seq.parts_name[:10] + \
                    '...' if len(seq.parts_name) > 12 else seq.parts_name
    ...
    # equivalent of 2.0 dpl_dna_designs.csv
    def write_dna_parts_order(self, filepath) -> None:
        """
    ...
    # equivalent of 2.0 plot_parameters.csv
    def write_plot_params(self, filepath) -> None:
        """
