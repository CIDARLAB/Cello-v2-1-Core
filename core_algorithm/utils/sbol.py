...
class SBOL:
    ...
    def generate_xml(self):
        ...
        circuit = sbol3.Component('circuit', sbol3.SBO_DNA, roles=[
                                  sbol3.SO_ENGINEERED_REGION])
        doc.add(circuit)
        ...
        for part in self.dna_seqs.values():
            ...
            components[c_id] = sbol3.Component(
                f'{c_id}', sbol3.SBO_DNA, roles=role)
            doc.add(components[c_id])

            # TODO: Correct Encoding?
            sequences[s_id] = sbol3.Sequence(
                f'{s_id}', elements=seq, encoding=sbol3.IUPAC_DNA_ENCODING)
        ...
        for part in self.part_order:
            ...
            if part.endswith('_NONCE_PAD'):
                ...
                lp_id = f'{name}'
                landing_pads[lp_id] = sbol3.Component(
                    f'Landing_Pads/{lp_id}', sbol3.SBO_DNA)
                ...
                ranges[r_id] = sbol3.Range(
                    sequences[s_id], start, end, identity=range_uri)  # TODO: Fix ranges

                subcomponents[c_id] = sbol3.SubComponent(
                    components[c_id], name=f'SubComponent_{c_id}')
