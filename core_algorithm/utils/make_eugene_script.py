...
class EugeneStruct:
    ...
    gates_struct: str = ""
    """from 'gates' block in UCF"""
    struct_cassettes: list[str, str, int, list[str]] = field(
        default_factory=list[str, str, int, list[str]])
...
class EugeneCassette:
    ...
    cir_rules: list[str] = field(
        default_factory=list[str])  # TODO: Account for other types of circuits not ALL_FORWARD
...
class EugeneObject:
    ...
    def __init__(self, ucf, in_map, gate_map, out_map, best_graphs):
        ...
        # always present; *all* associated parts used  # and terminator?
        self.parts_types: [str] = ['spacer', 'scar']
        # usually 'L1', 'L2', etc.
        self.genlocs_fenceposts: dict[str, List[str]] = {}
    ...
    def generate_eugene_structs(self) -> bool:
        ...
        for rnl_gate, g_gate in self.gate_map:
            for g_in in g_gate.inputs:
                for rnl_input, g_input in self.in_map:
                    if rnl_input[1] == g_in:
                        # TODO: Still need to merge multi-inputs?
                        edges[g_gate.gate_in_use].append(g_input.name)
        ...
        i_structures = self.ucf.query_top_level_collection(
            self.ucf.UCFin, 'structures')
        ...
        gate_devices = self.ucf.query_top_level_collection(
            self.ucf.UCFmain, 'gates')
        g_structures = self.ucf.query_top_level_collection(
            self.ucf.UCFmain, 'structures')
        for k, v in self.structs_dict.items():
                                ...
                                if d['name'].endswith('_cassette'):
                                    ...
                                    for c in d['components']:
                                        if c.startswith('#in'):
                                            ...
                                            print(
                                                'ERROR: UNRECOGNIZED COMPONENT IN DEVICE IN STRUCTURES IN UCF')
                                    if cassette[2] > 0:
                                        cassette[0] = d['name']
        ...
        o_structures = self.ucf.query_top_level_collection(
            self.ucf.UCFout, 'structures')
        for k, v in self.structs_dict.items():
    ...
    def generate_eugene_cassettes(self) -> bool:
        ...
        for in_part in in_parts:
            ...
            if p in ins or t in ['scar']:
                ...
                self.parts_seq_dict[p] = EugeneSequence(
                    t, p, in_part['dnasequence'], ['ALL_FORWARD'], [])
        main_parts = self.ucf.query_top_level_collection(
            self.ucf.UCFmain, 'parts')
        for main_part in main_parts:
            ...
            if p in mains or t in ['scar']:
                if t not in self.parts_types:
                    self.parts_types.append(t)
                self.parts_seq_dict[p] = EugeneSequence(
                    t, p, main_part['dnasequence'], ['ALL_FORWARD'], [])
        out_parts = self.ucf.query_top_level_collection(
            self.ucf.UCFout, 'parts')
        for out_part in out_parts:
            ...
            if p in outs or t in ['scar']:
                if t not in self.parts_types:
                    self.parts_types.append(t)
                self.parts_seq_dict[p] = EugeneSequence(
                    t, p, out_part['dnasequence'], ['ALL_FORWARD'], [])
    ...
    def generate_eugene_helpers(self) -> Tuple[Dict, Dict, Dict, Dict[str, Any], Dict[str, Any], Dict[str, List[str]]]:
        ...
        def extract_rules(rule_sets, func_type='AND') -> list[str]:
            ...
            if type(rule_sets) == list:
                ...
                log.cf.error(
                    'rule_set type is unrecognized; cannot parse or flatten rule_sets.')
            return flat_rules
        ...
        genetic_locations = self.ucf.query_top_level_collection(
            self.ucf.UCFmain, 'genetic_locations')[0]
        ...
        # Get Device Rules
        in_d_rules = init_extraction(
            self.ucf.query_top_level_collection(self.ucf.UCFin, 'device_rules'))
        gate_d_rules = init_extraction(
            self.ucf.query_top_level_collection(self.ucf.UCFmain, 'device_rules'))
        out_d_rules = init_extraction(
            self.ucf.query_top_level_collection(self.ucf.UCFout, 'device_rules'))
        ...
        for r in self.device_rules:
            # if r != 'ALL_FORWARD':
            operands = [word for word in r.split(
            ) if word not in rules_keywords]
        ...
        # Get Circuit Rules
        in_c_rules = init_extraction(
            self.ucf.query_top_level_collection(self.ucf.UCFin, 'circuit_rules'))
        gate_c_rules = init_extraction(
            self.ucf.query_top_level_collection(self.ucf.UCFmain, 'circuit_rules'))
        out_c_rules = init_extraction(
            self.ucf.query_top_level_collection(self.ucf.UCFout, 'circuit_rules'))
        ...
        for r in self.circuit_rules:
            # if r != 'ALL_FORWARD':
            operands = [word for word in r.split(
            ) if word not in rules_keywords and not word.startswith('[')]
            if len(operands) == 0:
    ...
    def write_eugene(self, filepath: str) -> bool:
        ...
        with open(filepath, 'w') as eug:
            ...
            for s in self.parts_seq_dict.values():
                eug.write(
                    f"{s.parts_type} {s.parts_name}(.SEQUENCE(\"{s.parts_sequence}\"));\n")
            eug.write('\n')
            ...
            # TODO: Add 'Device' after each device name/operand in each rule...
            ...
            for device in self.structs_cas_dict.keys():
                eug.write(
                    f"for(num i{i} = 0; i{i} < sizeof({device}_devices); i{i} = i{i} + 1) {{}} \n")
