...
class DNADesign:
    ...
    def write_regulatory_info(self, filepath) -> None:  # FIXME: issue with SC1...
        """
    ...
    def write_dna_sequences(self, filepath) -> None:
        ...
        with open(dna_sequences, 'w', newline='') as dna_seq:
            ...
            for order in self.valid_circuits:
                ...
                for seq in order:
                    if seq == '_NONCE_PAD':
                        segments.append(f'Segment {count}')
                        count += 1
    ...
    # NOTE: Native Python script for generating part orders (alternative to miniEugene) ################################
    ...
    #         return consolidated_devices
    #
    #     def transfer_part_colors(self) -> None:
    #         """
    #
    #         """
    #         for device in self.cassettes:
    ...
    #
    #             if not d_left and [part for part in d_placed if part] not in self.valid_dev_orders:
    #                 self.valid_dev_orders.append([part for part in d_placed if part])
